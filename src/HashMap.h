...
typedef enum Map_Result {
	...
	MAP_FUNC_NULL_ERROR				/**< User function is null      */
} Map_Result;


typedef size_t (*HashFunction)(const void* _key); /* yotam - 1 const added */
typedef int (*EqualityFunction)(const void* _firstKey, const void* _secondKey); /* yotam - 2 const added */
 ...
 */
void HashMap_Destroy(HashMap** _map, void (*_keyDestroy)(const void* _key), void (*_valDestroy)(const void* _value)); /* yotam - added const to _key and to _value */
 ...
 * 
 * @warning key must be unique and destinct
 * @warning - yotam - added const to _pKey and to _pValue.
 */
 ...
 * @param[in] _map - Hash map to remove pair from, must be initialized /yotam - fix - not remove
 * @param[in] _key - key to serve as index 
 * @param[in] _pValue - pointer to variable that will get the value if found.
 * @return Success indicator
 * @retval  MAP_SUCCESS	on success
 ...
 * 
 * @warning key must be unique and destinct
 * @warning - yotam - const added to _pValue
 */
